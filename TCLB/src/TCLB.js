...
const HAS_100K_LLAMAS = ['aenea-jones', 'damaimikaz', 'luke-crowe', 'timing2', 'ioulaum', 'championx91', 'thegalleryofeve'];
...
try {
  ...
  const $forEach = (array, callback) => {
    if (array && callback) { for (let i = 0; i < array.length; i++) callback(array[i], i); }
  };
  const addCSS = (css) => {
  ...
  let pubSubCount;
  let askedServer;
  const getGiveMenu = (devName, { load, error }) => {
  ...
  const askServerIfSuccess = (devName) => {
    getGiveMenu(devName, {
      load() {
        const success = this.response.includes('Already gave a Llama');
        setButtonsState(devName, (success ? 'success' : 'error'));
        if (success) { localStorage[`${loggedInDev}|${devName}`] = 0; }
      },
      error() {
  ...
  const showModal = (devName, url) => {
    const html = `<div id="modalspace" style="display:block">
<div class="modal-cel">
...
<div id="modalfade" style="display:block"></div>`;
  ...
  const llamaButtonClicked = function () {
  ...
  const askServerForStatus = (llamaButton, devName) => {
    if ({}.hasOwnProperty.call(llamaButtonsToUpdate, devName)) {
      ...
      const callback = (devID, className, title) => {
        if (className !== 'unknown') {
          lastStates[devName] = { className, title };
          if (devID) devIDs[devName] = devID;
        }
        if (className === 'already') { localStorage[`${loggedInDev}|${devName}`] = '0'; }
        llamaButtonsToUpdate[devName].forEach((button) => {
      ...
      getGiveMenu(devName, {
        load() {
          const regexResult = /data-userid=\\"(\d+?)\\"/.exec(this.response);
          if (!regexResult) {
            callback(0, 'unknown', TITLES.unknown.err_dev_id);
            return;
          }
          const devID = regexResult[1];
  ...
  const heading = 'Two Click Llama Button encountered an error:\n';
