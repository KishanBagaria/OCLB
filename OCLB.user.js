...
addJS(function() {
    ...
    try {
        ...
        var llamaButtonClicked = function(event) {
            ...
            if (!$includes(['give', 'error', 'spam', 'token_miss'], this.className.slice(10))) return; // 10 === 'oclb oclb-'.length
        ...
        const getCsrfToken = async () => {
            ...
            // Last resort: try to get from session storage (some DA pages store it there)
            try {
        ...
        var clearCsrfTokenCache = function() {
            csrfTokenCache = null;
            csrfTokenCacheTime = 0;
            storage('set', 'cached_csrf', null);
        };

        // Detect account changes
        var checkForAccountChange = function() {
            var currentUser = getLoggedInDeviantName();
            var savedUser = storage('get', 'oclb_last_user');
            ...
            if (currentUser && currentUser !== savedUser) {
                ...
                csrfTokenCacheTime = 0;
                storage('set', 'cached_csrf', null);
                storage('set', 'oclb_last_user', currentUser);

                // Clear existing button states
                lastStates = {};

                // Re-initialize all llama buttons on the page
                var buttons = document.querySelectorAll('span.oclb');
        ...
        // Add periodic checking for account changes
        setInterval(checkForAccountChange, 5000); // Check every 5 seconds
        ...
        // Find usernames that have avatar images nearby
        var findUsernamesWithAvatars = function() {
            var usernameElements = [];
            var processedUsernames = new Set();

            // Find all avatar images (img tags)
            var avatarImages = document.querySelectorAll('img[alt*="avatar"], img[alt$="\'s avatar"]');

            // Find all SVG default avatars with aria-label or role="img"
            var svgAvatars = document.querySelectorAll('svg[aria-label*="avatar"], svg[aria-label$="\'s avatar"], [role="img"][aria-label*="avatar"], [role="img"][aria-label$="\'s avatar"]');

            var processAvatar = function(avatarElement, username) {
                if (!username || processedUsernames.has(username)) return;

                // Find the closest text element that contains this username
                var searchContainer = avatarElement;
                ...
                for (var level = 0; level < 5; level++) {
                    ...
                    var textElements = searchContainer.querySelectorAll('span, a, div, p, h1, h2, h3, h4, h5, h6');
            ...
            avatarImages.forEach(function(img) {
                ...
                var match = altText.match(/^([a-zA-Z0-9][a-zA-Z0-9-_.]*)'s avatar$/);
                if (match) {
            ...
            svgAvatars.forEach(function(svg) {
                ...
                var titleElement = svg.querySelector('title');
                if (titleElement) {
                    var titleText = titleElement.textContent;
                    var titleMatch = titleText.match(/^([a-zA-Z0-9][a-zA-Z0-9-_.]*)'s avatar$/);
                    if (titleMatch) {
        ...
        var addLlamaButtonToUsername = function(usernameData) {
            var usernameElement = usernameData.element;
            var devName = usernameData.username;

            if (!usernameElement || usernameElement.getAttribute('data-oclb-added')) return;
            ...
            // Check for existing button anywhere in the parent hierarchy
            // Look for any span.oclb with matching devName within the same comment container
            var commentContainer = usernameElement.closest('.iGTCKt, .TzdmPT, [class*="comment"]');
            if (commentContainer) {
                var existingButton = commentContainer.querySelector('span.oclb[devName="' + devName + '"]');
                if (existingButton) {
                    // Button already exists, don't add another
                    return;
                }
            }

            // Also check nearby elements (within 3 parent levels)
            var checkElement = usernameElement;
            for (var i = 0; i < 3; i++) {
                if (!checkElement.parentElement) break;
                checkElement = checkElement.parentElement;
                var nearbyButton = checkElement.querySelector('span.oclb[devName="' + devName + '"]');
                if (nearbyButton) {
                    return;
                }
            }
            ...
            llamaButton.setAttribute('devNameReg', devName);
            llamaButton.setAttribute('data-oclb-added', 'true');
        ...
        // Watch for dynamically added widgets and usernames
        var watchForWidgets = function() {
            // Initial scan
            var usernameData = findUsernamesWithAvatars();
            usernameData.forEach(addLlamaButtonToUsername);
            ...
            var observer = new MutationObserver(function(mutations) {
                ...
                if (needsCheck) {
                    setTimeout(function() {
                        var newUsernameData = findUsernamesWithAvatars();
                        newUsernameData.forEach(addLlamaButtonToUsername);
                    }, 100);
                }
            });
        ...
        var addLlamaButton = function(devNameLink) {
            ...
            // Don't add if button already exists nearby
            var parentContainer = devNameLink.closest('.iGTCKt, .TzdmPT, [class*="comment"], [data-role="widget"]');
            if (parentContainer) {
                var existingButton = parentContainer.querySelector('span.oclb[devName="' + devName + '"]');
                if (existingButton) {
                    return; // Button already exists from widget detection
                }
            }
        ...
        var addLlamaButtonsInDA = function() {
            ...
            // Start watching for widget usernames with avatars
            watchForWidgets();
