...
addJS(function() {
    ...
    try {
        ...
        var setButtonsState = function(devName, className, title, dontTellOtherTabs) {
            ...
            llamaButtons.forEach(function(llamaButton) {
        ...
        var llamaButtonClicked = function(event) {
            ...
            var validStates = ['give', 'error', 'spam', 'token_miss'];
            if (!validStates.includes(this.className.slice(10))) return;
        ...
        var addFooterLinks = function() {
            ...
            var LINKS = {
                'One Click Llama Button': '//kishan.org/userscripts/one-click-llama-button/',
            };
        ...
        var addLlamaButtonsInDA = function() {
            ...
            var addEverywhere = function() {
                ...
                var selectors = [badgesLinkSelector, usernameLinkSelector];

                if (document.querySelectorAll(watchersSelector).length > 0) selectors.push(watchersSelector);
                if (document.querySelectorAll(watchingSelector).length > 0) selectors.push(watchingSelector);
                if (document.querySelectorAll(membersSelector).length > 0) selectors.push(membersSelector);
                if (document.querySelectorAll(adminSelector).length > 0) selectors.push(adminSelector);

                waitForElements(document.body, selectors.join(','), addLlamaButton);
            };
            ...
            if (window.location.href.includes('/preferences/')) {
                ...
                addMessageListener(function(data) {
                    if (data.loggedInDev) {
                        ...
                        document.querySelectorAll('span.oclb').forEach(function(button) {
